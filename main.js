...
function makeTime() {
    switch(state.mode) {
        ...
        case 'startStopWatch':
            {const d = new Date(new Date().getTime() - state.startTime.getTime());
        ...
        default: //startTimer
           { const d = new Date(state.timer - new Date().getTime() + state.startTime.getTime());
            return `${d.getUTCHours() < 10 ? "0" + d.getUTCHours().toString() : d.getUTCHours()}:${d.getUTCMinutes() < 10 ? '0' + d.getUTCMinutes().toString() : d.getUTCMinutes()}:${d.getUTCSeconds() < 10 ? '0' + d.getUTCSeconds().toString() : d.getUTCSeconds()}`}
    }
}
...
function startTimer() {
    ...
    changeButtons();
    state.startTime = new Date();
    state.timer = (document.getElementById("hours").value * 3600 + document.getElementById("minutes").value * 60 + document.getElementById("seconds").value) * 1000;
    changeClock(makeTime());
    interval = setInterval(() => changeClock(makeTime()), 1000);
}
